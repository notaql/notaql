 ...
 */
public class JSONInputPathParser implements InputPathParser {
    private final TransformationParser transformationParser;

    public JSONInputPathParser(TransformationParser parser) {
    ...
    public InputPath parse(String enginePath, boolean relative) {
        ...
        final List<InputPathStep> steps = inputPathContext.inputPathStep()
                .stream()
                .map(s -> new JSONInputPathStepVisitor(this.transformationParser).visit(s))
